...
class Ex01 {
    ...
    String compute(int qtdLivros) {
        ...
        final double critA = (0.25 * qtdLivros) + 7.50;
        final double critB = (0.50 * qtdLivros) + 2.50;

        if (critA > critB) return "Criterio B";
        if (critA < critB) return "Criterio A";
        if (critA == critB) return "Indiferente";
